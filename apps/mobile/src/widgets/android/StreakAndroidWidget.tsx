...
export const StreakAndroidWidget = ({
  ...
  const ink =
    state.kind === 'milestone' && state.milestone !== null
      ...
      : family === 'small'
        ? theme.ink
        : theme.inkML;
  // 디자인 카드(158×158 / 338×158 / 338×354dp) 좌표를 위젯 실측 크기에 비율 환산한다
  ...
  return (
    // 아트는 모서리까지 채운 사각형이고 라운드 클리핑은 런처가 한다 — 별도 배경 레이어가 필요 없다
    <OverlapWidget
      clickAction="OPEN_APP"
      style={{ width, height, backgroundColor: theme.bg as `#${string}` }}
    >
      {art != null ? (
        <ImageWidget
          image={art}
            ...
            {week.labels.map((label, index) => (
              ...
              >
                {week.done[index] && fruit != null ? (
                  <ImageWidget image={fruit} imageWidth={16} imageHeight={16} />
                ) : (
