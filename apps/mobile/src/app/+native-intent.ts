// OAuth 콜백·위젯 딥링크가 라우터 화면 전환(Unmatched Route)으로 이어지지 않게 막는다
...
import { isWidgetEntryUrl } from '../widgets/widget-link';
...
export function redirectSystemPath({
  ...
  try {
    return isOAuthRedirectUrl(path) || isWidgetEntryUrl(path) ? null : path;
  } catch {
    return path;
  }
}
