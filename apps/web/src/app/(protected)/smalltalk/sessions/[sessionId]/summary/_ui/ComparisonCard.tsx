...
// 가로 막대 하나 — 길이는 둘 중 큰 값 기준. 0에서 그 길이까지 차오른다
const Bar = ({
  ratio,
  value,
  tone,
  ...
  <div className="flex items-center gap-2">
    <div className="h-2 flex-1 overflow-hidden rounded-full bg-secondary">
      <div
        className={`animate-bar-fill h-full rounded-full ${tone === 'current' ? 'bg-primary' : 'bg-border'}`}
        style={
          {
            '--bar-width': `${ratio === 0 ? EMPTY_BAR_PERCENT : ratio * 100}%`,
            '--i': order,
          } as CSSProperties
        }
      />
