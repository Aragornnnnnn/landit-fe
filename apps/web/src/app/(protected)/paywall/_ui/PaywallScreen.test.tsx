// 페이월 화면 동작 — 플랜을 바꾸면 CTA·안내가 따라 바뀌고, 닫기·복원·결제 시작이 제자리로 간다
...
vi.mock('../_model/request-purchase', () => ({
  requestPurchase: mocks.requestPurchase,
  requestRestore: mocks.requestRestore,
}));
...
describe('PaywallScreen', () => {
  ...
  it('CTA를 누르면 고른 플랜으로 결제 시작 이벤트를 찍고 결제를 요청한다', () => {
    render(<PaywallScreen />);
    ...
    expect(mocks.requestPurchase).toHaveBeenCalledWith('yearly');
  });
  ...
  it('구매 복원을 누르면 복원 이벤트를 찍고 복원을 요청한다', () => {
    ...
    expect(mocks.requestRestore).toHaveBeenCalledTimes(1);
  });
