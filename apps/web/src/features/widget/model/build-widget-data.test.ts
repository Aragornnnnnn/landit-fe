...
describe('buildWidgetData — 주간 완료 창', () => {
  ...
  it('최근 7일이 두 달에 걸치면 이번 달·지난달 달력을 합쳐 채운다', () => {
    ...
    const data = buildWidgetData(streakOf({ today: '2026-08-03' }), null, [
  ...
  it('오늘 칸은 /me/streak가 단일 출처다 — 달력에 오늘이 적혀 있어도 activeToday가 이긴다', () => {
    const data = buildWidgetData(streakOf({ activeToday: false }), null, [
...
describe('buildWidgetData — 오늘 카드', () => {
  ...
  it('오늘 카드 제목이 비어 있으면 null로 보낸다 — 빈 문자열은 브릿지 스키마가 거부해 위젯 데이터 전체가 버려진다', () => {
    const data = buildWidgetData(streakOf({ currentStreakDays: 3 }), {
