...
결제 전 유저 설문. 한 사람이 여러 번 낼 수 있고, 응답은 `survey_responses`에 한 행씩 쌓인다. 설문이 끝나면 통째로 걷어낸다.
...
2. `rg LAN-428 apps/web`에 걸리는 줄 정리 — page-view의 `'survey'`, 편지 상세(`LetterDetailFlow`)의 CTA 한 줄과 import, `.env.example`의 설문 항목
