...
// 분석 대기 연출 — 로딩을 "듣기→비교→검토→정리" 서사로 풀어 래디 포즈와 문구가 단계마다 바뀐다.
// 기본 문구는 발음 평가용이고, 대화 직후 레벨 분석처럼 같은 리듬이 필요한 곳은 문구만 바꿔 쓴다.
...
const PRONUNCIATION_STAGES: AnalyzingStage[] = [
  {
...
export const AnalyzingLandy = ({
  ...
  return (
    <div className="flex w-full flex-col items-center gap-4">
      ...
      {/* 리전 노드는 고정 — 리전과 내용이 함께 삽입되면 스크린 리더가 변경을 놓친다 */}
      <p role="status" aria-live="polite" className="text-base font-medium">
        <span key={stage} className="animate-pop inline-block">
