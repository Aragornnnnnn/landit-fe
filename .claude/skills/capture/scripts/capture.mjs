...
const PRESETS = {
  pr: { viewport: { width: 375, height: 812 }, deviceScaleFactor: 2, statusInset: 0 },
  figma: { viewport: { width: 390, height: 832 }, deviceScaleFactor: 3, statusInset: 58 },
};
...
const findPlaywright = () => {
  ...
  throw new Error('playwright-core를 못 찾았다. 스크래치패드에서 `npm i playwright-core` 후 그 폴더에서 실행할 것');
};
const { chromium } = createRequire(findPlaywright())('playwright-core');

const chromeArgs = ['--autoplay-policy=no-user-gesture-required'];
if (flag('mic')) chromeArgs.push('--use-fake-device-for-media-stream', '--use-fake-ui-for-media-stream');
...
await ctx.addInitScript((statusInset) => {
  ...
  style.textContent =
    ...
    (statusInset ? ` [style*="safe-area-inset-top"]{padding-top:${statusInset}px!important}` : '');
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        state: {
          ...
          member: { userId: 1, provider: 'KAKAO', email: 'capture@landit.im', nickname: '캡처' },
        },
        version: 0,
      }),
    );
...
const steps = stepsFile ? (await import(pathToFileURL(resolve(stepsFile)).href)).default : null;
