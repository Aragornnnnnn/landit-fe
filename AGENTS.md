...
- **수동 메모이제이션(useCallback/useMemo/React.memo)을 쓰지 않는다** — web(next.config)·mobile(app.json) 모두 React Compiler가 켜져 있어 자동 처리된다.
- **구현은 TDD로** — 새 로직·버그 수정은 테스트를 먼저 쓰고 통과시키며 구현한다. 시키지 않아도 기본값이 TDD다. 작성 규칙은 [docs/testing.md](docs/testing.md) 참고 (Given/When/Then 구조, 한국어 행동 문장 이름, 분기·계약 위주로 테스트).
