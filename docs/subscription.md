...
카드 표기는 2026-09-07에 정했다. 두 카드 모두 월 기준으로 비교한다 — 월간은 월 19,800원 취소선에 월 9,900원, 연간은 월 9,900원(월간 실판매가) 취소선에 월 4,990원, 부제에 실제 청구액 연 59,900원. 배지는 두 카드 위 테두리에 걸치고 고른 카드만 골드다. 피그마 결정 ①②는 이걸로 닫혔다.
...
- 월 환산가는 59,900 ÷ 12 = 4,991.67원을 10원 단위로 내려 "월 4,990원"으로 표기한다. 환산값이라 실제 청구액 연 59,900원을 부제에 함께 적는다. (반올림 4,992와 비교해 논의했고, 읽기 쉬운 4,990으로 결정)
...
구성은 위에서부터 상태바, 닫기(좌), 로고+PREMIUM 배지, 단체샷 히어로, 혜택 6줄, 플랜 카드 2장, CTA, 하단 안내, 이용약관·개인정보 처리방침·구매 복원 링크다. 구매 복원과 약관 링크는 애플 심사 요구사항이라 빼지 않고 하단 링크 줄에 둔다.

정해진 스타일은 다음과 같다.

- 선택 카드는 주황 2px 테두리와 `#FFFBF8` 배경. 체크 아이콘은 쓰지 않는다. 안 고른 카드는 흰 배경에 회색 1px.
- 연간이 기본 선택. 배지는 두 카드 모두 상단 테두리에 절반 걸치고, 고른 카드의 배지만 골드 그라데이션(미선택은 회색). 월간 "출시 기념 50%", 연간 "월간보다 50% 저렴".
- 카드 안 순서는 제목, 취소선(월 기준 비교가), 큰 가격(월 기준), 부제 한 줄. 두 카드가 같은 단위를 쓴다.
...
| 연간 카드 부제 | 연 59,900원 · 7일 무료 체험                     | 같음                                      |
